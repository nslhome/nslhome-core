...
var Provider = function() {
    ...
    this.initialize = function (providerType, providerName, callback) {
        ...
        mongo.connect(env.MONGO_URL, {db: {native_parser: false}}, function(err, db) {
            ...
            db.collection('providers').find({'name': me.providerName}).limit(1).next(function(err, provider) {
                if (err) {
                    return callback(err);
                }

                if (provider === null) {
                    return callback(new Error("Unknown provider '" + me.providerName + "'"));
                }
    ...
    this.send = function(message) {
        ...
        if (message.name == "device") {
            devices.find({_id: message.body.id}).limit(1).next(function(err, device) {
                ...
                if (device == null)
                    device = {body: {}, provider: me.providerName, _id: message.body.id};
